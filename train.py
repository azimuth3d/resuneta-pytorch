...
from loss import IoULoss,DiceLoss,tanimoto_loss,TverskyLoss
##
...
class Train:
    ...
    def train(self):
        ...
        dir_result_train = os.path.join(self.dir_result, self.scope, name_data,
                                        'train')
        if not os.path.exists(os.path.join(dir_result_train, 'images')):
            os.makedirs(os.path.join(dir_result_train, 'images'))

        dir_result_val = os.path.join(self.dir_result, self.scope, name_data,
                                      'val')
        ...
        transform_train = transforms.Compose([RandomCrop((self.ny_load, self.nx_load)), RandomFlip(), ToTensor()])

        # transform_train = transforms.Compose([ToTensor()])

        # transform_train = transforms.Compose([ Normalize(), ToTensor()])

        # transform_train = transforms.Compose([RandomFlip(), Normalize(), ToTensor()])
        ...
        dataset_train = Dataset(dir_data_train,
                                data_type=self.data_type,
                                transform=transform_train)
        dataset_val = Dataset(dir_data_val,
                              data_type=self.data_type,
        ...
        num_batch_val = int((num_val / batch_size) +
                            ((num_val % batch_size) != 0))
        ...
        for epoch in range(st_epoch + 1, num_epoch + 1):
                ...
                if should(num_freq_disp):
                    ...
                    plt.imsave(
                        os.path.join(dir_result_train, 'images',
                                     fileset['input']), input[0].squeeze())
                    plt.imsave(os.path.join(dir_result_train, 'images',
                                            fileset['label']),
                               label[0].squeeze(),
                               cmap='gray')
                    plt.imsave(os.path.join(dir_result_train, 'images',
            ...
            writer_train.add_scalar('loss_G_cls', np.mean(loss_G_cls_train),
                                    epoch)
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(loader_val, 1):
                    ...
                    if should(num_freq_disp):
                        ...
                        plt.imsave(os.path.join(dir_result_val, 'images',
                                                fileset['input']),
                                   input[0].squeeze(),
                                   cmap=cmap)
                        plt.imsave(os.path.join(dir_result_val, 'images',
                ...
                writer_val.add_scalar('loss_G_cls', np.mean(loss_G_cls_val),
                                      epoch)
    ...
    def test(self):
        ...
        dir_result_test = os.path.join(self.dir_result, self.scope, name_data,
                                       'test')
        ...
        dataset_test = Dataset(dir_data_test,
                               data_type=self.data_type,
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader_test, 1):
                ...
                for j in range(label.shape[0]):
                    ...
                    output_crf = np.concatenate((1 - output_crf, output_crf),
                                                axis=0)

                    U = unary_from_softmax(output_crf)

                    d = dcrf.DenseCRF2D(output_crf.shape[1],
                                        output_crf.shape[2],
                                        output_crf.shape[0])
                    d.setUnaryEnergy(U)
                    ...
                    pairwise_energy = create_pairwise_gaussian(
                        sdims=(sdims, sdims), shape=(input_.shape[:2]))
                    d.addPairwiseEnergy(pairwise_energy, compat=compat)
                    ...
                    output_crf = map_soln_unary.reshape(
                        (output_crf.shape[1], output_crf.shape[2]))

                    ##
                    plt.imsave(os.path.join(dir_result_test_save,
                                            fileset['input']),
                               input_.squeeze(),
                               cmap=cmap)
                    plt.imsave(os.path.join(dir_result_test_save,
                ...
                print('TEST: %d/%d: LOSS: %.6f' %
                      (i, num_batch_test, loss_G_cls.item()))
